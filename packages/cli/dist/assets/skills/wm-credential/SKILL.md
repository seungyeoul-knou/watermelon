---
name: wm-credential
description: Watermelon credential management skill. Securely registers, updates, and deletes external service credentials (API keys, tokens) in Watermelon. This skill should be used when the user says "/wm-credential", "add credential", "register API key", or wants to manage credentials in Watermelon.
user_invocable: true
---
...
- `/wm-credential` → Show action selection menu.
- `/wm-credential add <service name>` → Start the add credential flow.
- `/wm-credential list` → Show registered credentials list.
...
When designing workflows (`/wm-design`, `/wm-improve`), set the credential on the node that calls an external API:
