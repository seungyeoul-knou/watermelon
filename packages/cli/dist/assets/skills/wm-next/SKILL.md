---
name: wm-next
description: Watermelon resume skill. Finds the active running task and resumes execution from the current step. Use when the user says "/wm-next", "next step", "next", "continue", "proceed", "이어서", "계속", or wants to resume a running Watermelon task from where it left off.
user_invocable: true
---
...
**Use `/wm-next`** when: there is at least one `running` task and the user wants to continue it.
**Use `/wm-start`** when: starting a new workflow, picking from multiple candidates, or resuming a **specific** task by ID (`/wm-start #123`).

## Core Principles

Shared with wm-start. Summary:
...
If the user says "proceed", "next", "continue", "let's go", "OK", "go ahead", "이어서", "계속" — treat it the same as `/wm-next`.
...
4. If **no running tasks** → call `list_tasks(status=timed_out)` and check for timed-out tasks.
   ...
   - If none → tell the user "실행 중인 태스크가 없습니다. `/wm-start`로 새 워크플로를 시작하세요."
...
Same contract as wm-start. When calling `execute_step`:
...
Same contract as wm-start: when `advance` returns a `credentials` field, use `credentials.secrets.<KEY>` as headers/params (e.g. `Authorization: Bearer $ACCESS_TOKEN`). Never write raw secret values into `execute_step` output — only the call result (URL, status, response summary).
...
Do not show "type /wm-next to continue" hint.
...
2. Show the user:
   ...
   A notification has been sent. Use /wm-approve when ready.
   ```
3. STOP. Do NOT call `advance`. Do NOT proceed to the next step.

The server will reject `advance` with 403 until a human approves via /wm-approve.
</HARD-RULE>
...
- **HITL approval required** (execute_step returned `next_action: "wait_for_human_approval"`):
  Call `request_approval`, show waiting message, stop. Resume only after `/wm-approve`.
- **Gate step**: Wait for user response via AskUserQuestion.
- **Other action step pausing without HITL**: "Type `/wm-next` to proceed."
...
> ⚠️ 태스크가 웹 UI에서 중지되었습니다. Task #{id} — Step {N}에서 사용자에 의해 중단되었습니다. 다시 시작하려면 `/wm-next`로 재개하거나 `/wm-start`로 새 워크플로를 시작하세요.
