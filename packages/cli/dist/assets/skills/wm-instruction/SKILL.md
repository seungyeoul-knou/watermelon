...
- `/wm-instruction` → Show action selection menu.
- `/wm-instruction add <title>` → Start creating a new instruction.
- `/wm-instruction list` → Show instruction list.
- `/wm-instruction move <id> <folder id>` → Move an instruction between folders.
...
If no match: "Could not find a matching credential. Register it first with `/wm-credential add`."
...
when designing nodes with `/wm-design`.
```
