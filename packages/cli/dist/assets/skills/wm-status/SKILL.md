---
name: wm-status
description: Watermelon task status skill. Checks the progress of active and completed tasks. This skill should be used when the user says "/wm-status", "task status", "check progress", or wants to view Watermelon task status.
user_invocable: true
---
...
- `/wm-status` → Show all running tasks. If none, show recent completed tasks.
- `/wm-status <task_id>` → Show detailed step-by-step log for that task.
- `/wm-status running` → Show only running tasks.
- `/wm-status completed` → Show only completed tasks.
...
- header: "Actions"
- options: ["Resume active task (/wm-start)", "View detailed task", "Done"]
