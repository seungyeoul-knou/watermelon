...
export function vscodeGlobalStorage(extensionId) {
    const home = homedir();
    switch (platform()) {
        case "darwin":
            return join(home, "Library", "Application Support", "Code", "User", "globalStorage", extensionId);
        case "win32":
            return join(process.env.APPDATA ?? join(home, "AppData", "Roaming"), "Code", "User", "globalStorage", extensionId);
        default:
...
export function jetbrainsConfigDir() {
    const home = homedir();
    switch (platform()) {
        case "darwin":
...
export function traeUserDir() {
    const home = homedir();
    switch (platform()) {
        case "darwin":
            return join(home, "Library", "Application Support", "Trae", "User");
        case "win32":
            return join(process.env.APPDATA ?? join(home, "AppData", "Roaming"), "Trae", "User");
        default:
