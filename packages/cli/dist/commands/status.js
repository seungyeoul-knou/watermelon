...
export async function statusCommand(profileName) {
    ...
    if (local.record) {
        console.log(`${pc.bold("Local runtime:")} ${local.running ? pc.green("running") : pc.red("stopped")}`);
        console.log(`${pc.bold("Local URL:")}     ${local.record.url}`);
        console.log(`${pc.bold("Local PID:")}     ${local.record.pid}`);
        console.log(`${pc.bold("Local runtime kind:")} ${local.record.runtimeKind} (${local.record.runtimeSource})`);
        ...
        if (local.stale) {
            ...
            console.log(`${pc.bold("Local health:")}  ${local.healthy
                ? pc.green(`ok${local.healthStatus ? ` (${local.healthStatus})` : ""}`)
                : pc.red(local.healthStatus
