---
name: wm-improve
description: Watermelon workflow improvement skill. Analyzes an existing workflow, creates a better version, and optionally runs it to compare results. This skill should be used when the user says "/wm-improve", "improve workflow", "make a better version", or wants to refine an existing Watermelon workflow.
user_invocable: true
---
...
- **`/wm-improve`** (this one) — a registered workflow exists and needs a better version.
- **`/wm-design`** — start from a natural-language goal without an existing workflow.
- **`/wm-import`** — external resource (GitHub repo, URL, local skill, pasted text) to convert into a new workflow.

## Argument Handling

- `/wm-improve` → Fetch workflow list, ask user to select.
- `/wm-improve <workflow name>` → Load that workflow directly.
...
If "Run now" → switch to `/wm-run` flow.
