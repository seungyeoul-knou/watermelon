---
name: wm-share
description: Watermelon sharing skill. Shares a folder or workflow with a user group at a specified access level. This skill should be used when the user says "/wm-share", "share workflow", "share folder", or wants to share Watermelon content with a group.
user_invocable: true
---
...
- `/wm-share` → Show resource and group selection.
- `/wm-share <folder name>` → Pre-select the folder and ask for group/access level.
...
To revoke access later, use `/wm-share` and select "Remove share".
```
